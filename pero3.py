...
#replicacao de supercelula 2x2x1
...
#z_medio =
z_third = c_uni/n_camada #correspondente a metade da altura. A celula é dividida em tres camadas. n/3 Ms na camada 1 e n/2 na camada 2
...
n2 = 0 #contador de indice para vetor de Ms que possuem o z na media
n3 = 0 #contador de indice para vetor de Ms que possuem o z abaixo da media
...
for i in range(no_M):
    ...
    ##print('M_',i)
    #loop
    if (z_M[i] > 2*z_third) and (z_M[i] < 3*z_third):
        
        #M esta na camada n_camada 1
        M_camada_index[0][n1] = i
        n1 += 1
                    
    elif (z_M[i] > 1*z_third) and (z_M[i] < 2*z_third):
...
##camada n3: (z_X < z_M_medio_n3) ; (z_X > z_M_medio_n3) & (z_X < z_M_medio_n2) 
...
##print('z_medio por camada: \n',z_medio_por_camada)
##print('camda n2/n3: ',z_min_n3,'< z_X <',z_max_n2,'')

##print('--> [camda n1]',X_camadas_index_b1)
##print('--> [camda n1/n2]',X_camadas_index_b2)
##print('--> [camda n2/n3]',X_camadas_index_b3)
##print('--> [camda n3]',X_camadas_index_b4)
...
#print('Vetor dos X vale:               ',X_camadas_index[3][:])
...
##print(index_X_camadas)
#matriz dos X fora do plano equatorial ab
range_toral = 4*index_X_camadas #len(X_camadas_index[0][:]) + len(X_camadas_index[1][:]) + len(X_camadas_index[2][:] + len(X_camadas_index[3][:]))
X_api_index = np.zeros(range_toral)

##print('range toral=',range_toral_eq)
...
##print('\n Matirix de M: \n',x_M,'\n',y_M,'\n',z_M) 
##print('\n Matirix de X: \n',X_equa_x,'\n',X_equa_y,'\n',X_equa_z)
##print('\n Matirix coordenadas de M (x,y,z,): \n',coord_atom_M) 
...
for i in range(busca_X_M):
    ...
    #identificando qual seria o atomo M_i (da matriz das coordenadas de X proximas a um M)
...
#print('Distancias X-M de inter-face')
#print('* no plano equatorial: \n',intf_X_M_equa)
#print('* na direcao equatorial: \n',intf_X_M_api)
...
#print('Distancias M-X-M de inter-face')
#print('* no plano equatorial: \n',intf_M_X_M_equa)
    ...
    if (i != 0) and (i != 2):
        ...
        for j in range(colunas_M):
            ...
            for g in range(no_M_X_M_equa):
                ...
                ##print('Camada n',i+1,'>> angulo',M_X_M_equa[g][4]) ####################### parei aqui!
                ##print('M',M_camada_index[i][j],'== M',M_X_M_equa[g][0],'?')
                #se os indices M forem iguais (M_camada_index[i][j] == dist_X_M_equa[g][0]):
                #aloca-se a coluna dist_X_M_equa[g][1]=dist
                if (M_camada_index[i][j] == M_X_M_equa[g][0]):
...
#print('Ângulos X-M de entre-camada')
#print('* no plano equatorial: \n',entc_M_X_M_equa)
#print('* na direcao apical: \n',entc_M_X_M_api)
...
#print('\nPlano equatorial na inter-face')
resultado_intf_M_X_M_equa = max_min_array(intf_M_X_M_equa)
...
#print('\nDirecao apical na inter-face')
resultado_intf_M_X_M_api = max_min_array(intf_M_X_M_api)
...
#with open("output_XM_n3_interf.dat", "w") as f:
#with open("GeI_XM_n3_interf.dat", "w") as f:
#with open("GeBr_XB_n3_interf.dat", "w") as f:
with open("XM_n3_interf.dat", "w") as f:
#with open("Sn_XM_n3_interf.dat", "w") as f:
...
# Criando arquivo xyz com os resultado
csv.register_dialect('myDialect', delimiter = ' ', quoting=csv.QUOTE_NONE, skipinitialspace=True)
#with open("output_XM_n3_entrec.dat", "w") as f:
#with open("Ge_XM_n3_entrec.dat", "w") as f:
#with open("GeI_XM_n3_entrec.dat", "w") as f:
#with open("GeBr_XB_n3_entrec.dat", "w") as f:
with open("XM_n3_entrec.dat", "w") as f:
#with open("Sn_XM_n3_entrec.dat", "w") as f:
    writer = csv.writer(f, dialect='myDialect')
    writer.writerow(data)
...
# Criando arquivo xyz com os resultado
csv.register_dialect('myDialect', delimiter = ' ', quoting=csv.QUOTE_NONE, skipinitialspace=True)
#with open("output_MXM_n3_interf.dat", "w") as f:
#with open("GeI_MXM_n3_interf.dat", "w") as f:
#with open("GeBr_BXB_n3_interf.dat", "w") as f:
with open("MXM_n3_interf.dat", "w") as f:
#with open("Sn_MXM_n3_interf.dat", "w") as f:
    writer = csv.writer(f, dialect='myDialect')
    writer.writerow(data)
...
# Criando arquivo xyz com os resultado
csv.register_dialect('myDialect', delimiter = ' ', quoting=csv.QUOTE_NONE, skipinitialspace=True)
#with open("output_MXM_n3_entrec.dat", "w") as f:
#with open("Ge_MXM_n3_entrec.dat", "w") as f:
#with open("GeI_MXM_n3_entrec.dat", "w") as f:
#with open("GeBr_BXB_n3_entrec.dat", "w") as f:
with open("MXM_n3_entrec.dat", "w") as f:
#with open("Sn_MXM_n3_entrec.dat", "w") as f:
    writer = csv.writer(f, dialect='myDialect')
    writer.writerow(data)
...
for ocataedro_M in range(no_M): # separa distâncias r por átomo de metal M
    ...
    for dists0 in range(len(dist_X_M_equa)): # varre linha por linha da matriz (r,M)
...
Cabec = [[n_Atomos],[]]
csv.register_dialect('myDialect', delimiter = ' ', quoting=csv.QUOTE_NONE, skipinitialspace=True)
with open("interf_entrec_delta_d_n_3.dat", "w") as f:
...
Cabec = [[n_Atomos],[]]
csv.register_dialect('myDialect', delimiter = ' ', quoting=csv.QUOTE_NONE, skipinitialspace=True)
with open("interf_entrec_theta_n_3.dat", "w") as f:
    writer = csv.writer(f, dialect='myDialect')
