...
#z_medio =
z_fourth = c_uni/n_camada #correspondente a metade da altura. A celula é dividida em tres camadas. n/3 Ms na camada 1 e n/2 na camada 2
...
for i in range(no_M):
    ...
    ##print('M_',i)
    #loop
    if (z_M[i] > 3*z_fourth) and (z_M[i] < 4*z_fourth):
        
        #M esta na camada n_camada 1
        M_camada_index[0][n1] = i
        n1 += 1
                    
    elif (z_M[i] > 2*z_fourth) and (z_M[i] < 3*z_fourth):
        
        #M esta na camada n_camada 2
        M_camada_index[1][n2] = i
        n2 += 1
        
    elif (z_M[i] > 1*z_fourth) and (z_M[i] < 2*z_fourth):
...
#print('Vetor dos X vale:               ',X_camadas_index[4][:])
...
with open("XM_n4_interf.dat", "w") as f:
...
with open("XM_n4_entrec.dat", "w") as f:
...
with open("MXM_n4_interf.dat", "w") as f:
...
with open("MXM_n4_entrec.dat", "w") as f:
...
for ocataedro_M in range(no_M): # separa distâncias r por átomo de metal M
    ...
    for dists0 in range(len(dist_X_M_equa)): # varre linha por linha da matriz [octaedro, d_{X-M}]
        ...
        if ocataedro_M==dist_X_M_equa[dists0][0]: # identificando a distancia pelo numero do octaedro
            ...
            ##print(" inicio",Xs0)
            #if ocataedro_M==0:#print("loop",dists0,";",ocataedro_M,"# octaedro; somatoria =",soma_dists0,"(+",dist_X_M_equa[dists0][1],")")
            Xs0=Xs0+1
            ##print(" fim",Xs0)
    ...
    for dists1 in range(len(dist_X_M_api)): # vare linha por linha da matriz (r,M)
        ...
        if ocataedro_M==dist_X_M_api[dists1][0]:
            ...
            ##print(" inicio",Xs1)
            #if ocataedro_M==0:#print("loop",dists1,";",ocataedro_M,"# octaedro; somatoria =",soma_dists1,"(+",dist_X_M_api[dists1][1],")")
            Xs1=Xs1+1
            ##print(" fim",Xs1)
...
for ocataedro_M in range(no_M): # separa distâncias r por átomo de metal M
    ...
    for dists0 in range(len(dist_X_M_equa)): # vare linha por linha da matriz (r,M)
        ...
        if ocataedro_M==dist_X_M_equa[dists0][0]:
            ...
            #if ocataedro_M==0:#print("loop",dists0,";",ocataedro_M,"# octaedro;")#print("*somatoria =",soma_dists0,"=(",dist_X_M_equa[dists0][1],"-",somas_por_M[ocataedro_M][1],")/",somas_por_M[ocataedro_M][1],"^2")
            Xs0=Xs0+1
    ...
    for dists1 in range(len(dist_X_M_api)): # vare linha por linha da matriz (r,M)
        ...
        if ocataedro_M==dist_X_M_api[dists1][0]:
            ...
            ##print("Metal, soma de distâncias api:",ocataedro_M,soma_dists1)
            #if ocataedro_M==0:#print("loop",dists1,";",ocataedro_M,"# octaedro; somatoria =",soma_dists1)#print("*somatoria =",soma_dists1,"=(",dist_X_M_api[dists1][1],"-",somas_por_M[ocataedro_M][1],")/",somas_por_M[ocataedro_M][1],"^2")
            Xs1=Xs1+1
    ...
    #if ocataedro_M==0:#print("\n soma_equa(",soma_dists0,")+soma_api(",soma_dists1,")=",soma_dists0+soma_dists1)#print("\n Delta_d=",soma_dists0+soma_dists1,"/",Xs0+Xs1,"=",desvio_d_por_M[ocataedro_M][1])
#print("\n",desvio_d_por_M)
...
Cabec = [[n_Atomos],[]]
csv.register_dialect('myDialect', delimiter = ' ', quoting=csv.QUOTE_NONE, skipinitialspace=True)
with open("interf_entrec_delta_d_n_4.dat", "w") as f:
...
Cabec = [[n_Atomos],[]]
csv.register_dialect('myDialect', delimiter = ' ', quoting=csv.QUOTE_NONE, skipinitialspace=True)
with open("interf_entrec_theta_n_4.dat", "w") as f:
    writer = csv.writer(f, dialect='myDialect')
